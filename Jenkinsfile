            ...
            steps {
                withCredentials([file(credentialsId: 'azure-ssh-key-file', variable: 'KEY_PATH')]) {
                    powershell """
                        ...
                        # Added 'kubectl apply -f service.yaml' to ensure the networking door is open
                        & C:\\Windows\\System32\\OpenSSH\\ssh.exe -i \$keyPath -o StrictHostKeyChecking=no ${env.VM_USER}@${env.VM_IP} "
